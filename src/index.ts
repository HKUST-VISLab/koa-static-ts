...
export function serve(root: string = '', opts: StaticServerOptions = {}) {
    ...
    const options = Object.assign({}, staticServerDefaultOptions, opts);
    if (options.defer) {
    ...
    return async (ctx: Context, next: () => Promise<any>) => {
        ...
        if (ctx.method === 'HEAD' || ctx.method === 'GET') {
            try {
                done = await send(ctx, root, options);
            } catch (err) {
                if (err.status !== 404) {
                    throw err;
                }
 ...
 * @api public
 */
async function send(ctx: Context, rootPath: string, opts: StaticServerOptions & typeof staticServerDefaultOptions) {
